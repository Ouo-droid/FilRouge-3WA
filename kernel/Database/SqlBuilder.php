...
class SqlBuilder
{
    ...
    {
        $data = method_exists($class, 'toDatabaseArray')
            ? $class->toDatabaseArray()
            : self::extractObjectProperties($class);
    ...
    public static function prepareUpdate(object $class, string $table, string $primaryKey = 'id'): array
    {
        $data = method_exists($class, 'toDatabaseArray')
            ? $class->toDatabaseArray()
            : self::extractObjectProperties($class, true);
        ...
        $sql = sprintf(
            ...
            $setClause,      // Clause SET avec les colonnes
            $primaryKey,     // Nom de la clé primaire
            $primaryKey      // Nom de la clé primaire pour le paramètre
        );
     ...
     * ]
     */
    public static function prepareDelete(string $table, int|string $id, string $primaryKey = 'id'): array
    {
    ...
    public static function sanitize(array $data, ?callable $customSanitizer = null): array
    {
        ...
        return array_map(function ($value) use ($customSanitizer) {
            ...
            // Les valeurs null sont conservées telles quelles pour que PDO les transmette
            // correctement comme SQL NULL (et non comme chaîne vide).
            if (null === $value) {
                return null;
            }
